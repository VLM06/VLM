// ==========================================
// VLM ERP SYSTEM v2.0 - COMPLETE UPGRADE
// Invoice, Expense & Bank Ledger Management
...
const db = new sqlite3.Database(process.env.DB_PATH || './vlm_database.db', (err) => {
    if (err) {
        ...
        console.log('✅ Connected to VLM Database v2.0');
    }
});
...
db.serialize(() => {
    ...
    const migrations = [
        ...
        `ALTER TABLE bank_ledger ADD COLUMN reference_type TEXT`,
        `ALTER TABLE bank_ledger ADD COLUMN reference_id INTEGER`,
        `ALTER TABLE bank_ledger ADD COLUMN reconciled INTEGER DEFAULT 0`
    ];
...
const initializeGemini = async () => {
    try {
        ...
        console.log('⚠️ Gemini AI not available (optional)');
        return null;
    }
};
...
initializeGemini().then(model => {
    geminiModel = model;
    if (geminiModel) console.log('✅ Gemini AI Ready');
});
...
const getAIInsights = async (query, context) => {
    ...
    try {
        const response = await geminiModel.generateContent(
            `You are a financial advisor. Based on this business data: ${JSON.stringify(context)}. Answer concisely: ${query}`
        );
                ...
                db.get(
                    ...
                    (err, bankRow) => {
                        ...
                        res.json({
                            status: 'success',
                            metrics: {
                                bankBalance: bankRow.bank_balance || 0,
                                pendingReceivables,
                                totalHours,
...
app.patch('/api/invoices/:id/pay', (req, res) => {
    ...
    if (amount <= 0) return res.status(400).json({ status: 'error', message: 'Invalid amount' });
...
app.listen(PORT, () => {
    console.log(`
${'='.repeat(60)}`);
    console.log(`✅ VLM ERP v2.0 RUNNING`);
    console.log(`🌐 Server: http://localhost:${PORT}`);
    console.log(`📊 Dashboard: http://localhost:${PORT}/`);
    console.log(`📚 Docs: Check README.md`);
